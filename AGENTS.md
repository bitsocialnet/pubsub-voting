...
Trustless pubsub voting library that runs on a host's shared libp2p/Helia node. The **engine and client lifecycle are implemented** (schemas, canonical encoding, topic/manifest derivation, the `PubsubVoter` facade, the CRDT, transport, verify, tally, chain reads, the **client-level republish scheduler**, and durable vote-intent persistence via Node SQLite / browser IndexedDB — `VoteNetwork.start`/`castVotes`/`getTally` and the full `PubsubVoter.start`/`stop`/`destroy` lifecycle are live). What remains is the **live-delta transport migration** (DESIGN.md's migration note is the shipped-vs-target ledger): the pubsub payload moves from winner-CID lists + bitswap fetch to a two-kind union — one inline bundle per message, or a tiny **root record** — with root-record checkpoint sync (libp2p-fetch pull + topic heartbeat, blocks via directed bitswap) and a binary bundle block encoding. Read [DESIGN.md](./DESIGN.md) before changing anything.
...
- **The live-delta migration is the current work** (see ROADMAP "Remaining for v1"): inline-bundle + root-record pubsub union with a criteria-derived size cap; binary bundle block encoding (re-pins the bundle-CID and checkpoint test vectors — the EIP-712 vector MUST NOT change); on-demand checkpoint encode; root heartbeat (10 min ± jitter, suppression, one-per-interval); libp2p-fetch responder/requester with the `MissingFetchError` construction guard; directed-bitswap chase of divergent roots **through the same verifier**, so a single liar cannot inject a bad vote nor hide an honest one. See [DESIGN.md, Transport](./DESIGN.md) and [Checkpoints](./DESIGN.md#checkpoints).
- **Add a two-node gossipsub integration test** (real `@libp2p/gossipsub` **>= 15.0.23** — the floor that fixes the CVE-2026-46679 subscription-flood memory DoS; see DESIGN.md "Deferred pkc-js work" — gated behind an integration flag) pinning what a fake cannot prove: an invalid inline bundle is NOT forwarded and its sender is `reject`-scored (P₄); a valid bundle IS forwarded and merges on the peer; a verify that exceeds the 10s deadline yields `ignore` with no penalty; heartbeat suppression stays quiet on a converged pair; a divergent root triggers a directed-bitswap chase that converges the pair. The pure decision logic is unit-tested (`src/transport/gossip-validator.test.ts`) — this pins real-gossipsub timing.
