...
| Resolves to a bundle that fails verification (bad signature, `address` ≠ recovered signer, vote out of range, expired bucket, over `maxVotesPerAddress`, duplicate `board.publicKey` within one bundle, a board `name` that does not resolve to the claimed `publicKey`) | offline verify (layer 2) + chain/name resolution (layer 3) | dropped — never stored, re-served, or counted |
...
3. **Tally** (lazy, on-chain): eligibility and weight reads, plus board-`name` resolution through the host-injected `nameResolvers`, only for the votes that can still change the visible ranking.
...
**Name claims are resolved before a row is trusted.** A vote's `board.name` never keys a row (`publicKey` does), but it is a claim, and the tally verifies it alongside the other per-vote chain work: resolve the name through the host-injected `nameResolvers` (the same `{ key, provider, resolve, canResolve }` interface pkc-js takes, implemented by `@bitsocial/bso-resolver` — injected on the `PubsubVoter` constructor, so this library imports no resolver) and **drop the bundle when the name does not resolve or resolves to a different `publicKey` than the vote claims**. Only bundles whose name↔publicKey pairing matches the registry are aggregated, so a counted named vote is always truthful and two boards can never surface under one name — squatting a popular name with a freshly minted key just produces bundles that fail resolution. A vote may omit `name` entirely (a board with no registered name); it folds into its `publicKey` row unresolved. This check obeys the lazy-verify contract: it only subtracts (a dropped bundle lowers a ceiling, never raises a floor), and it is a network read, so it runs with the chain reads, after the free signature check has already pruned. Key rotation needs no special case — when an owner re-points a name to a new `publicKey`, bundles signed for the old pair start failing resolution and decay out through the liveness heartbeat re-sign (`ceil(voteExpiryBuckets / 2)` buckets), whose fresh resolution picks up the new key. A re-point therefore fully takes effect within one republish cycle — up to half an expiry window, not a single bucket — the wider window the coarser liveness cadence trades for.
...
**One kind, one return type: `{ score: bigint }`.** There is a single interpreter kind; `evaluate` returns a result object whose non-negative `score` of `0n` means "does not qualify". (It is an object, not a bare `bigint`, so a slot-specific field — e.g. a self-declared `ceiling` for balance-derived weight — can be added later without another signature change; see "Open questions".) The criteria keeps two *slots* that draw from the one registry: the **eligibility** slot treats the score as a gate (`> 0n` admits, `0n` rejects), the **weight** slot treats it as the vote's magnitude. Final vote value = `evaluate(eligibility).score === 0n ? 0n : evaluate(weight).score`. A separate boolean eligibility kind was considered and rejected for the simplicity of one kind/one registry: an interpreter that needs a threshold (min Passes, min balance) bakes it in by returning `0n` below the threshold (`erc721-min-balance` returns the holding or 0n; `erc20-balance` takes an optional `min`). The same interpreter can therefore serve either slot — e.g. `erc721-min-balance` is a Pass gate in eligibility and "1 vote per Pass" in weight. Each slot holds exactly one interpreter; combining several — an AND of gates, or an additive weight — is a deferred *structural* extension, not a combinator interpreter (see "Future improvements").
...
- **Pinned-block name resolution.** Board-name verification (see "Tally") should read the naming registry at a canonical historical block, exactly as balance reads are pinned to the bucket block — but the injected resolver interface (pkc-js's `NameResolverInterface`, implemented by `@bitsocial/bso-resolver`) resolves at head with no block parameter. And pinning is not just an API gap: the registry lives on its own chain (Ethereum for `.bso`) while the bucket boundary block is defined on the criteria's chain (Base in the 5chan example), so a deterministic read also needs a canonical per-bucket block *on the registry's chain* (per-chain bucket math, or mapping the bucket boundary timestamp to the nearest registry-chain block — the same multi-chain problem as the tie seed and the eligibility AND-array). Until both land, v1 resolves at head and accepts a disagreement window around a re-point: two verifiers can transiently disagree about a name whose record just changed — the same eventual consistency the CRDT accepts elsewhere, bounded in practice because records change rarely and the heartbeat re-signs each liveness cycle (`ceil(voteExpiryBuckets / 2)` buckets), so the window is at most one republish cycle, up to half the expiry window, not a single bucket. The block-parameter gap is tracked upstream in [bso-resolver#3](https://github.com/bitsocialnet/bso-resolver/issues/3).
