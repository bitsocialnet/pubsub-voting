 ...
 * a non-canonical change silently changes the topic. See DESIGN.md "Criteria document".
 */
...
export const CriteriaSchema = z
    .object({
        ...
        name: z.string().min(1),
        /** Directory-slot codes. Each vote's `contest` must be one of these. */
        contests: z.array(z.string().min(1)).nonempty(),
        /** Allowed range for each `vote` value. v1: { min: 1, max: 1 }. */
        voteSchema: VoteRangeSchema,
        /** Cap on votes per author per bundle (anti-spam). */
