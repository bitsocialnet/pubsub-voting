...
describe("makeTally", () => {
    it("sums constant weight and folds different names with the same key into one row", async () => {
        const bundles = [
            bundle("0xaaa", [{ board: { name: "memes.bso", publicKey: KEY_A }, vote: 1 }]),
            bundle("0xbbb", [{ board: { name: "funny.bso", publicKey: KEY_A }, vote: 1 }])
        ];
    ...
    it("ranks boards by weight, highest first", async () => {
        const bundles = [
            bundle("0xaaa", [{ board: { publicKey: KEY_A }, vote: 1 }]),
            bundle("0xbbb", [{ board: { publicKey: KEY_A }, vote: 1 }]),
            bundle("0xccc", [{ board: { publicKey: KEY_B }, vote: 1 }])
        ];
        const tally = await makeTally(makeDeps(bundles)).compute();

        expect(tally.ranking.map((r) => [r.board.publicKey, r.weight])).toEqual([
            [KEY_A, 2n],
            [KEY_B, 1n]
        ]);
    });
    ...
    it("breaks a weight tie via the rolling block-hash seed, deterministically", async () => {
        ...
        const bundles = [
            bundle("0xaaa", [{ board: { publicKey: KEY_A }, vote: 1 }]),
            bundle("0xbbb", [{ board: { publicKey: KEY_B }, vote: 1 }])
        ];
        ...
        // Both boards tie at weight 1, so the block hash is read to break it.
        expect(blockHashReads).toBeGreaterThan(0);
        expect(first.ranking.map((r) => r.board.publicKey)).toEqual(second.ranking.map((r) => r.board.publicKey));
        expect(new Set(first.ranking.map((r) => r.board.publicKey))).toEqual(new Set([KEY_A, KEY_B]));
    });
});
